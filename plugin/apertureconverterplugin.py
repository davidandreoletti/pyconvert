...
loggerJobCreationSuccessFormatter = logging.Formatter('%(message)s')
loggerJobCreationSuccessFileHandler = logging.FileHandler(__name__+"-creation-success-"+str(Constant.getLoggerTime())+".log", mode='w', encoding="UTF-8")
loggerJobCreationSuccessFileHandler.setFormatter(loggerJobCreationSuccessFormatter)
...
loggerJobCreationSkippedFileHandler = logging.FileHandler(__name__+"-creation-skipped-"+str(Constant.getLoggerTime())+".log", mode='w', encoding="UTF-8")
loggerJobCreationSkippedFileHandler.setFormatter(loggerJobCreationSuccessFormatter)
...
loggerJobCreationFailedFileHandler = logging.FileHandler(__name__+"-creation-failed-"+str(Constant.getLoggerTime())+".log", mode='w', encoding="UTF-8")
loggerJobCreationFailedFileHandler.setFormatter(loggerJobCreationFailedFormatter)
...
class ApertureConverterPlugin(ConverterPlugin):
	...
	def canConvert(self,filename):
		...
		fileBasenameWithoutExtension = os.path.basename(os.path.splitext(filename)[0].strip())
		fileExtensionLowerCase = fileExtension.lower()
		if fileExtensionLowerCase in ApertureConverterPlugin.SUPPORTED_FILE_EXTENSION:
			return True
		else:
			return False
	...
	def createVideoJob(self, sourceMediaFile=None):
		...
		if countSourceVideoStreams == 1:
			...
			if countSourceVideoStreams == 1:
				...
				logger.warn("NOTE:"+sourceMediaFile.getFileName()+": "+str(countSourceVideoStreams)+" audio stream found.")
			...
			if( sVWidth >= 1280 and sVHeight >= 720 and 
               ...
               sABitPerSample == "16"):
				dVHandbrakeCodecName=HandbrakeCLGenerator.ENCODER_VIDEO_FORMAT_H264
				dVHandbrakeStreamIndex="0"
				dVHandbrakeQuality="25"
				dVHandbrakeWidth=1280
				dVHandbrakeHeight=720
				dAHandbrakeCodecName=HandbrakeCLGenerator.ENCODER_AUDIO_FORMAT_FAAC
				hanbrakeDestinationMediaFile.setContainerType(HandbrakeCLGenerator.CONTAINER_FORMAT_MP4)
			...
			# Unknown 1
				...
				skippedJobCreation = True
				skippedReason = "No suitable converter configuration found. Missing a convertion case ? New video file ?"
			...
			if not skippedJobCreation:
				try:
					...
					destinationHandbrakeVideoStream = VideoStream(codecName=dVHandbrakeCodecName, streamIndex=dVHandbrakeStreamIndex, framerateMin=sVFramerateMin, width=dVHandbrakeWidth, height=dVHandbrakeHeight, quality=dVHandbrakeQuality)
					destinationHandbrakeAudioStream = AudioStream(streamIndex=dAHandbrakeStreamIndex, sampleRate=dAHandbrakeSampleRate, codecName=dAHandbrakeCodecName)
				...
				loggerJobCreationSkipped.info("JOB_CREATION_SKIPPED:"+sourceMediaFile.getFileName()+": Type:"+loggerCameraSourceType+" Reason: "+skippedReason)			
		else:
	...
	def createVideoStream(d=None):
		...
		return VideoStream(codecLongName=ApertureConverterPlugin.getValueOrDefault(d, "codec_long_name"),codecTag=ApertureConverterPlugin.getValueOrDefault(d, "codec_tag"),streamIndex=ApertureConverterPlugin.getValueOrDefault(d, "index"),codecTagString=ApertureConverterPlugin.getValueOrDefault(d, "codec_tag_string"),codecName=ApertureConverterPlugin.getValueOrDefault(d, "codec_name"),framerateMin=framerateMinValue, width=ApertureConverterPlugin.getValueOrDefault(d, "width"), height=ApertureConverterPlugin.getValueOrDefault(d, "height"), quality=None)
	...
	def createAudioStream(d=None):
		...
		return AudioStream(codecLongName=ApertureConverterPlugin.getValueOrDefault(d, "codec_long_name"),codecTag=ApertureConverterPlugin.getValueOrDefault(d, "codec_tag"), codecTagString=ApertureConverterPlugin.getValueOrDefault(d, "codec_tag_string"), codecName=ApertureConverterPlugin.getValueOrDefault(d, "codec_name"), streamIndex=ApertureConverterPlugin.getValueOrDefault(d, "index"),bitPerSample=ApertureConverterPlugin.getValueOrDefault(d, "bits_per_sample"),sampleRate=sampleRateValue)
	...
	def probeImage(self, filename=None):
			...
			imageStream = ImageStream(filename=filename,width=width, height=height, quality=quality, densityX=dpiX, densityY=dpiY)
			imageStreams = dict()
