...
class ConvertJobCreator(object):
    ...
    def createJobFromMediaDirectory(self, mediaDirectoryPath, videoFileExtensions, imageFileExtensions):
        """
            Create a job containing all jobs to execute. Each job when execute converts 
            a media file from one file format to another.
        ...
        for root, dirs, files in os.walk(os.path.abspath(mediaDirectoryPath), topdown=False):
            for name in files:
        ...
        for filename in filenames:
            ...
            fileBasenameWithoutExtension = basename(os.path.splitext(filename)[0].strip())
            fileExtensionLowerCase = fileExtension.lower()
            # Accept only non hidden files with specific file extension 
            if len(fileBasenameWithoutExtension) > 0 and len(fileExtensionLowerCase) > 0:
